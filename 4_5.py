...
import random
kustik = int(input("введите количество кустиков: "))
yagodi = list(random.randint(0, 10) for i in range(kustik))
print(*yagodi)
...
while (i < kustik):
    if (i == kustik - 1):
        summa = yagodi[0] + yagodi[i] + yagodi[i - 1] 
    else:
        summa = yagodi[i] + yagodi[i + 1] +yagodi[i - 1] 
        result.append(summa)
        result.sort()
    i += 1

print(f"число ягод составляет: {result[-1]}")
